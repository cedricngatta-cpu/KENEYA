    ...
    return (
        <div className="p-8 space-y-8 animate-in fade-in duration-500 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-slate-500 font-medium text-sm mt-2 flex items-center gap-2">
                        ...
                        Connecté : <span className="font-bold text-slate-700">{centerName}</span> • Surveillance active
                    </p>
                </div>
                <div className="flex gap-3">
                    <Link href="/dashboard/map" className="px-5 py-3 bg-slate-900 text-white font-black text-xs uppercase tracking-widest rounded-2xl flex items-center gap-2 hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/10">
                        <MapPin size={16} className="text-emerald-400" /> Radar SIG
                    </Link>
                    <button onClick={fetchData} disabled={refreshing} className="p-3 bg-white border border-slate-200 rounded-2xl hover:bg-slate-50 transition-all group">
                        <RefreshCw size={18} className={`text-slate-400 group-hover:text-emerald-600 transition-colors ${refreshing ? 'animate-spin' : ''}`} />
                    </button>
                    <button
                        onClick={() => setShowNewCase(true)}
                        className="px-6 py-3 bg-emerald-600 text-white font-black text-xs uppercase tracking-widest rounded-2xl flex items-center gap-2 hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-600/20 active:scale-95"
                    >
            ...
            {activeClusters.length > 0 && (
                <div className="bg-red-50 border border-red-100 rounded-[2rem] p-6 relative overflow-hidden group animate-in slide-in-from-top-4 duration-700">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <h2 className="text-xl font-black text-red-900">Alerte Épidémique Majeure</h2>
                                <p className="text-red-700/70 text-sm font-medium">L'IA a détecté {activeClusters.length} cluster(s) actif(s) dans le Grand Abidjan.</p>
                            </div>
                        </div>
                        <div className="flex flex-wrap gap-2">
                            {activeClusters.slice(0, 3).map((cluster) => (
                                <div key={cluster.id} className="bg-white/80 backdrop-blur-md border border-red-200 px-4 py-2 rounded-xl flex items-center gap-3 shadow-sm">
                                    <span className="w-2 h-2 rounded-full bg-red-600 animate-ping"></span>
                                    <span className="text-xs font-black text-red-900 uppercase tracking-tighter">{cluster.geo_cell}</span>
                                    <span className="h-4 w-px bg-red-100"></span>
                                    <span className="text-xs font-bold text-slate-500">{cluster.syndrome}</span>
                                </div>
                            ))}
                        </div>
                        <Link href="/dashboard/map" className="px-6 py-3 bg-red-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-red-700 transition-all flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {[
                    ...
                    <div key={i} className={`bg-white p-6 rounded-[2rem] border ${k.border} flex items-center gap-4 shadow-sm ${k.glow} hover:-translate-y-1 transition-all`}>
                        <div className={`p-4 rounded-2xl ${k.color}`}><k.icon size={24} /></div>
                        <div>
                            <div className="text-3xl font-black text-slate-900 tracking-tighter">{k.value}</div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div className="xl:col-span-2 space-y-6">
                    ...
                    {cases.length > 0 ? (
                        <div className="space-y-4">
                            {cases.map((c) => (
                                <div key={c.id} className="bg-white p-6 rounded-[2rem] border border-slate-100 flex items-center justify-between group hover:border-emerald-200 hover:shadow-xl hover:shadow-emerald-900/5 transition-all duration-300">
                                    <div className="flex items-center gap-5">
                                        <div className={`p-4 rounded-2xl ${severityColor(c.severity)} shadow-sm`}>
                                            <HeartPulse size={24} />
                                        </div>
                                        <div>
                                            <div className="font-black text-lg text-slate-900 leading-tight mb-1">{c.syndrome}</div>
                                            <div className="flex items-center gap-3">
                                                <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase border ${severityColor(c.severity)} shadow-sm`}>
                                                    {c.severity}
                                                </span>
                                                <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase ${statusColor(c.status)} shadow-sm`}>
                                                    {c.status}
                                                </span>
                                                <span className="text-[10px] text-slate-400 font-bold flex items-center gap-1.5 ml-1">
                                                    <Clock size={12} className="text-slate-300" /> {formatTime(c.created_at)}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0">
                                        {c.status === 'suspect' && (
                                            <button onClick={() => handleUpdateStatus(c.id, 'probable')}
                                                className="px-4 py-2 bg-orange-50 text-orange-600 text-[10px] font-black uppercase rounded-xl hover:bg-orange-100 transition-all border border-orange-100 shadow-sm">
                                                Qualifier Probable
                                            </button>
                                        )}
                                        {(c.status === 'suspect' || c.status === 'probable') && (
                                            <button onClick={() => handleUpdateStatus(c.id, 'confirmed')}
                                                className="px-4 py-2 bg-red-50 text-red-600 text-[10px] font-black uppercase rounded-xl hover:bg-red-100 transition-all border border-red-100 shadow-sm">
                                                Confirmer Cas
                                            </button>
                                        )}
                                    </div>
                ...
                <div className="space-y-6">
                    ...
                    {recentReports.length > 0 ? (
                        <div className="space-y-4 max-h-[70vh] overflow-auto no-scrollbar pr-2">
                            {recentReports.map((r) => (
                                <div key={r.id} className={`bg-white p-5 rounded-[2rem] border-l-[6px] border border-slate-100 shadow-sm transition-all hover:scale-[1.02] ${r.severity === 'rouge' ? 'border-l-red-500' : r.severity === 'jaune' ? 'border-l-orange-400' : 'border-l-green-500'}`}>
                                    <div className="flex items-center justify-between mb-3">
                                        <span className={`px-2 py-1 rounded-lg text-[9px] font-black uppercase border ${severityColor(r.severity)}`}>
                                            {r.severity}
                                        </span>
                                        <span className="text-[10px] text-slate-400 font-black font-mono bg-slate-50 px-2 py-1 rounded-md">{formatTime(r.created_at)}</span>
                                    </div>
            ...
            {selectedReport && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[60] flex items-center justify-center p-4 animate-in fade-in duration-300 pointer-events-auto" onClick={() => setSelectedReport(null)}>
                    <div className="bg-white rounded-[2.5rem] w-full max-w-lg p-8 shadow-2xl space-y-6 relative overflow-hidden ring-1 ring-slate-100" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 className="text-xl font-black text-slate-900">Notification <span className="text-emerald-600">IA</span></h2>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Communication Patient Intelligente</p>
                                </div>
                            </div>
